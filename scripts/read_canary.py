...
Every 10 minutes this opens a representative catalog dataset from its Icechunk
store on S3 and reads a single value, exercising the full icechunk -> S3 -> zarr
decode path. A successful read pings a Better Stack heartbeat; any failure (or a
hung read) simply skips the ping, so the heartbeat lapses and Better Stack opens
an incident.
...
# dynamical-catalog is pinned to the icechunk-2 branch to match this repo's
# pyproject source; switch to a PyPI version once that branch is released.
...
def read_canary() -> None:
    ...
    collection_id = os.environ.get("CANARY_COLLECTION_ID", "noaa-gfs-forecast")
    ...
    # Read succeeded — report liveness. Any exception above skips this line, so
    # the heartbeat lapses and Better Stack opens an incident.
