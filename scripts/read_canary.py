...
dataset. A successful read of every collection sends an `ok` Sentry cron
check-in; any failure (or a hung read) simply skips it, so the Sentry
monitor records a missed check-in and opens an incident.
